...
#pragma -- share image with suer
+ (void)shareUserEventually:(PFUser *)user image:(PFObject *)photo block:(void (^)(BOOL succeeded, NSError *error))completionBlock{
    if ([[user objectId] isEqualToString:[[PFUser currentUser] objectId]]) {
        return;
    }
    ...
    [followActivity setObject:[PFUser currentUser] forKey:kESActivityFromUserKey];
    [followActivity setObject:user forKey:kESActivityToUserKey];
    [followActivity setObject:photo forKey:kESActivityPhotoKey];
    [followActivity setObject:kESActivityTypeShare forKey:kESActivityTypeKey];
...
+ (void)unshareUserEventually:(PFUser *)user image:(PFObject *)photo{
    PFQuery *query = [PFQuery queryWithClassName:kESActivityClassKey];
    [query whereKey:kESActivityFromUserKey equalTo:[PFUser currentUser]];
    [query whereKey:kESActivityToUserKey equalTo:user];
    [query whereKey:kESActivityTypeKey equalTo:kESActivityTypeShare];
    [query whereKey:kESActivityPhotoKey equalTo:photo];
