...
static void * JWBlurViewPropertyKey = &JWBlurViewPropertyKey;
...
- (UIViewController *)_bluredView
{
    return objc_getAssociatedObject(self, JWBlurViewPropertyKey);
}
- (void)set_bluredView:(JWBlurView *)blurrview
{
    objc_setAssociatedObject(self, JWBlurViewPropertyKey, blurrview, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)presentPopUpView:(UIView *)popUpView completion:(void (^)(void))completionBlock
{
    ...
    JWBlurView *bluredView = [[JWBlurView alloc] initWithFrame:overlayView.bounds];
      self.en_popupViewController._bluredView = bluredView;
