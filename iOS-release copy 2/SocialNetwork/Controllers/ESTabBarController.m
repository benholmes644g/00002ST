...
//       // [editor view]
//        [[editor toolInfo] setAvailable:NO];
...
//
        ...
        //viewController.disablePin = YES;
       // viewController.audio=[editor.addedAudios copy];
        //viewController.audioPath=[editor.addedAudiosPath copy];
       // viewController.imgViewFrmSize = editor.frameSize;
...
//    if (cameraDeviceAvailable && photoLibraryAvailable) {
        NSArray *menuItems = @[[[RNGridMenuItem alloc] initWithImage:[UIImage imageNamed:@"ButtonCamera"] title:NSLocalizedString(@"Capture", nil)],
...
- (void)photoCaptureAction{
   ...
   // [self shouldStartCameraController];
    ...
    [self performSelector:@selector(shouldStartCameraController) withObject:nil afterDelay:0.1];
    return;
...
//    gridMenu.itemSize = CGSizeMake(100, 100);
//    [gridMenu showInViewController:self center:CGPointMake(self.view.bounds.size.width/2.f, self.view.bounds.size.height/2.f)];
//   // [gridMenu viewWillDisappear:YES];
//  //  - (void)gridMenu:(RNGridMenu *)gridMenu willDismissWithSelectedItem:(RNGridMenuItem *)item atIndex:(NSInteger)itemIndex {
//    [self gridMenu:gridMenu willDismissWithSelectedItem:[menuItems firstObject] atIndex:0];
//     //    } else {
//    //        // if we don't have at least two options, we automatically show whichever is available (camera or roll)
//    //        [self shouldPresentPhotoCaptureController];
    //    }
...
- (void)imageEditor:(_CLImageEditorViewController *)editor didFinishEdittingWithImage:(UIImage *)image
{
