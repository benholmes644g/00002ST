    ...
    if (text != nil) {
        ...
        [file saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            if (error == nil) {
               ...
               // [messengerItem setObject:@"picture" forKey:@"sidetone"];
...
- (void)imagePicker:(JSImagePickerViewController *)imagePicker didSelectImages:(NSArray *)images {
    ...
    [self sendMessage:nil withDict:image withVideo:nil andWithAudio:nil];
    }

}
- (void)sendMessage:(NSString *)text withDict:(NSMutableDictionary *)pictureDict withVideo:(NSURL *)video andWithAudio:(NSString *)audio {
    ...
    else if (pictureDict != nil) {
        [ProgressHUD show:@"Sending" Interaction:NO];
        PFFile *file = [PFFile fileWithName:@"picture.jpg" data:UIImageJPEGRepresentation([pictureDict objectForKey:@"img"], 0.6)];
        [file saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            if (error == nil) {
                [messengerItem setObject:file.url forKey:@"picture"];
                [messengerItem setObject:[NSString stringWithFormat:NSLocalizedString(@"%@ sent a picture", nil),[currentUser objectForKey:kESUserDisplayNameKey]] forKey:@"text"];
                [messengerItem setObject:@"picture" forKey:@"type"];
                [messengerItem setObject:[pictureDict objectForKey:@"photo"] forKey:@"sidetone"];
...
//
    UIActionSheet *action = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil
                                               otherButtonTitles:NSLocalizedString(@"Choose an Existing Sidetone", nil), nil];
    action.tag = 2;
...
- (void)collectionView:(JSQMessagesCollectionView *)collectionView didTapMessageBubbleAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([message.senderId isEqualToString:self.senderId] == YES) {
		...
		if ([[messengerItem objectForKey:@"status"] isEqualToString:@"Read"] == NO) {
            ...
            UIActionSheet *action = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:NSLocalizedString(@"Delete message", nil) otherButtonTitles:NSLocalizedString(@"View Sidetone", nil),nil];
            action.tag = 1;
            [action showInView:self.view];
			return;
		}
  	...
  	if (message.isMediaMessage) {
		if ([message.media isKindOfClass:[JSQPhotoMediaItem class]]) {
            NSDictionary *messengerItem = messengerItems[indexPath.item];
            NSString *objectid = [messengerItem objectForKey:@"sidetone"];
            
            PFObject *photo  = [PFObject objectWithoutDataWithClassName:kESPhotoClassKey objectId:objectid]; //[photo fetch];
            if (photo) {
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex {
     if (actionSheet.tag == 1) {
         if (buttonIndex == 0) {
             ...
             PFObject *photo  = [PFObject objectWithoutDataWithClassName:kESPhotoClassKey objectId:objectid]; [photo fetchIfNeeded];
             if (photo) {
                 ESPhotoDetailsViewController *photoDetailsVC = [[ESPhotoDetailsViewController alloc] initWithPhoto:photo];
                 [self.navigationController pushViewController:photoDetailsVC animated:YES];
