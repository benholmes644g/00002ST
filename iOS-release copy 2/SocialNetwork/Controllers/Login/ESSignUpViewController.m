...
- (BOOL)isValidPassword:(NSString*)password
{
    ...
    NSRange range = [password rangeOfCharacterFromSet:cset];
...
- (void)actionRegister
{
    ...
    if ([password length] < 8)	{ [ProgressHUD showError:NSLocalizedString(@"This password is not strong enough, passwords must be 8 characters and include one Uppercase character.", nil)]; return; }
    if (![self isValidPassword:password])	{ [ProgressHUD showError:NSLocalizedString(@"This password is not strong enough, passwords must be 8 characters and include one Uppercase character.", nil)]; return; }
