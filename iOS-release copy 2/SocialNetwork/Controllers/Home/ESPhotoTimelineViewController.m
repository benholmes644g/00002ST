...
@synthesize activityIndicator, tapOnce, tapTwice,objectsData,user1,user2;
...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        ...
        self.objectsData = [[NSMutableArray alloc]initWithCapacity:0];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    NSInteger sections = self.objectsData.count;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == self.objectsData.count) {
        // Load More section
        return nil;
    }
    ...
    PFObject *photoA = [self.objectsData objectAtIndex:section];
    
    
    PFObject *photo = [photoA objectForKey:kESActivityPhotoKey];//[self.objects objectAtIndex:section];
...
if(self.objects)
{
    ...
    for (PFObject* o in self.objects){
        ...
        // logic to ensure no photo is repeated
        
                        PFObject *p = [o objectForKey:kESActivityPhotoKey];
        ...
        [p fetch];
                                   ...
                                   // if(p){
                                       ...
                                       // [p fetch];
            PFUser *usr = [p objectForKey:kESPhotoUserKey];
            if(![([STParseHelper sharedInstance]).blockList containsObject:[usr objectId]]){

                                        [self.objectsData addObject:p];
            }
                            //  }
           ...
           /* BOOL willAddObject = true;
            if([temp count] <1){
               ...
               // [fromUser fetch];
               [temp addObject:[p objectId]];
              //  unreadCount++;
                [self.objectsData addObject:p];
                NSLog(@"********ADD PHOTO %@",[p objectId]);

               // NSLog(@"********ADD OBJECT %@",[fromUser objectId]);
            }else{
               // PFUser *fromUser = [activity objectForKey:kESActivityFromUserKey];
                //[fromUser fetch];
                if([temp containsObject:[p objectId]]){
                    willAddObject = false;
                    NSLog(@"********SKIP PHOTO %@",[p objectId]);
...
- (PFQuery *)queryForTable {
    ...
    // 0 from to 1 from 2 to 3 to from .. 4 from to to from
    
    PFQuery *sharedActivityQuery =[PFQuery queryWithClassName:kESActivityClassKey];
    [sharedActivityQuery whereKey:kESActivityTypeKey equalTo:kESActivityTypeShare];
   // [sharedActivityQuery includeKey:kESActivityPhotoKey];
    sharedActivityQuery.cachePolicy = kPFCachePolicyNetworkOnly;
    PFQuery *query;
    
    PFQuery *sharedByActivityQuery = [PFQuery queryWithClassName:kESActivityClassKey];
    [sharedByActivityQuery whereKey:kESActivityTypeKey equalTo:kESActivityTypeShare];
    //[sharedByActivityQuery includeKey:kESActivityPhotoKey];
    sharedByActivityQuery.cachePolicy = kPFCachePolicyNetworkOnly;
    ...
    switch (self.queryType) {
        case 0:
            ...
            if(self.user1)
             [sharedActivityQuery whereKey:kESActivityFromUserKey equalTo:self.user1];
            if(self.user2)
            [sharedActivityQuery whereKey:kESActivityToUserKey equalTo:self.user2];
            
            query = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects: sharedActivityQuery, nil]];
            [query includeKey:kESActivityPhotoKey];
            break;
            
        case 1:
            //user 1 is from and user2 is not defined
        ...
        case 2:
            //user 2 is from and user1 is not defined
        ...
        case 3:
            //
            [sharedActivityQuery whereKey:kESActivityFromUserKey equalTo:self.user2];
            [sharedActivityQuery whereKey:kESActivityToUserKey equalTo:self.user1];
        ...
        case 4:
            ...
            [sharedByActivityQuery whereKey:kESActivityFromUserKey equalTo:self.user2];
            [sharedByActivityQuery whereKey:kESActivityToUserKey equalTo:self.user1];
            
            query = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects: sharedActivityQuery, sharedByActivityQuery,nil]];
            [query includeKey:kESActivityPhotoKey];
...
//    if(self.from){
//        
//        [sharedActivityQuery whereKey:kESActivityFromUserKey equalTo:self.from];
//
//    }else if(self.to){
//       [sharedActivityQuery whereKey:kESActivityToUserKey equalTo:self.to];
//
//    }
//      [sharedActivityQuery includeKey:kESActivityPhotoKey];
//    sharedActivityQuery.cachePolicy = kPFCachePolicyNetworkOnly;
...
//        
//    }else{
//         [sharedByActivityQuery whereKey:kESActivityFromUserKey equalTo:[PFUser currentUser]];
//        
//    }
    
//    
//    PFQuery *query = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects: sharedActivityQuery,sharedByActivityQuery, nil]];
     [query orderByDescending:@"createdAt"];
//    
//     [query setCachePolicy:kPFCachePolicyNetworkOnly];
//    
//  
    if (self.objectsData.count == 0 || ![[UIApplication sharedApplication].delegate performSelector:@selector(isParseReachable)]) {
        [query setCachePolicy:kPFCachePolicyCacheThenNetwork];
    }
//    if(self.from){
//        
//         return sharedActivityQuery;
//
//    }else{
//    return sharedActivityQuery;
//    }
...
/*- (PFQuery *)queryForTable {
    ...
    PFQuery *sharedActivityQuery = [PFQuery queryWithClassName:kESActivityClassKey];
    [sharedActivityQuery whereKey:kESActivityTypeKey equalTo:kESActivityTypeShare];
   // [sharedActivityQuery whereKey:kESActivityToUserKey equalTo:[PFUser currentUser]];
   // [sharedActivityQuery includeKey:kESActivityPhotoKey];
    sharedActivityQuery.cachePolicy = kPFCachePolicyNetworkOnly;
    sharedActivityQuery.limit = 1000;

    PFQuery *photosSharedToUserQuery = [PFQuery queryWithClassName:self.parseClassName];
    //[photosSharedToUserQuery includeKey:kESActivityPhotoKey];
   // [sharedActivityQuery whereKey:kESPhotoPictureid matchesQuery:sharedActivityQuery];
    [sharedActivityQuery whereKey:kESPhotoPictureid matchesKey:kESActivityPhotoKey inQuery:sharedActivityQuery];
    //[sharedActivityQuery whereKeyDoesNotExist:@"type"];
   // [sharedActivityQuery whereKeyExists:kESPhotoPictureKey];
//[photosFromFollowedUsersQuery whereKey:kESPhotoPermission equalTo:@"public"];
   ...
   // [videosFromFollowedUserQuery whereKeyExists:kESVideoFileKey];
    ...
    [photosFromCurrentUserQuery whereKey:kESPhotoPermission equalTo:@"public"];
   ...
   // [videosFromCurrentUserQuery whereKeyExists:kESVideoFileKey];
    ...
    PFQuery *query = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects:photosFromFollowedUsersQuery, photosSharedToUserQuery, nil]];
...
- (PFObject *)objectAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section < self.objectsData.count) {
        return [self.objectsData objectAtIndex:indexPath.section];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object{
   ...
   // PFObject *object = [objects objectForKey:kESActivityPhotoKey];
    if (indexPath.section == self.objectsData.count) {
...
- (void)didTapOnPhotoAction:(UIButton *)sender {
    PFObject *photoA = [self.objects objectAtIndex:sender.tag];
    PFObject *photo = [photoA objectForKey:kESActivityPhotoKey];
    if (photo) {
        ESPhotoDetailsViewController *photoDetailsVC = [[ESPhotoDetailsViewController alloc] initWithPhoto:photo];
        [self.navigationController pushViewController:photoDetailsVC animated:YES];
