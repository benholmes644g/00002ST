...
-(void)viewWillAppear:(BOOL)animated {
   ...
   // self.navigationItem.leftBarButtonItem =delegate.container.
    ...
    self.sharedPhotos =[[NSMutableArray alloc] init];
    //Calculate Luminance
    CGFloat luminance;
...
- (PFObject *)objectAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < NUMBER_OF_STATIC_SECTIONS) {
        ...
        if([self.user isEqual:[PFUser currentUser]]){
            ...
            return [self.sharedPhotos objectAtIndex:indexPath.section-NUMBER_OF_STATIC_SECTIONS];
        }
       // return [self.objects objectAtIndex:indexPath.section-NUMBER_OF_STATIC_SECTIONS];
    }
    return nil;
...
- (PFQuery *)queryForTable {
    ...
    [query includeKey:kESPhotoUserKey];
        ...
        [query whereKey:kESActivityFromUserKey equalTo:self.user];
...
-(void)blockuser:(PFUser *)user_{
    //kESActivityTypeKey ===  kESActivityBlockKey; == block

    PFObject *object = [PFObject objectWithClassName:@"blocklist"];
     [object setObject:[PFUser currentUser] forKey:kESActivityFromUserKey];
    [object setObject:user_ forKey:kESActivityToUserKey];
    [object setObject:kESActivityBlockKey forKey:kESActivityTypeKey];
    
    [object saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        if (succeeded) {
            
            
            PFObject *object1 = [PFObject objectWithClassName:@"blocklist"];
            [object1 setObject:[PFUser currentUser] forKey:kESActivityToUserKey];
            [object1 setObject:user_ forKey:kESActivityFromUserKey];
            [object1 setObject:kESActivityBlockKey forKey:kESActivityTypeKey];
...
-(void)dismissController{
    ...
    [alert showNotice:self.tabBarController title:NSLocalizedString(@"Notice", nil)
     ...
     closeButtonTitle:@"OK" duration:10.0f];
...
- (void) ReportTap {
    UIActionSheet *actionSheet = [[UIActionSheet alloc]initWithTitle:NSLocalizedString(@"Do you want to report the user for infringing our terms of use?", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:NSLocalizedString(@"Yes, Report", nil) otherButtonTitles:NSLocalizedString(@"Block User", nil), nil];
    actionSheet.tag = 22;
    [actionSheet showInView:self.headerView];
}
-(void)showFollowers {
