...
- (void)viewDidLoad {
   ...
   // [navBarTapView addGestureRecognizer:gestureRecognizer];
    ...
    BOOL userl =[[[PFUser currentUser] objectForKey:@"firstLaunch"] isEqualToString:@"YES"];
    BOOL defal = [[[NSUserDefaults standardUserDefaults] objectForKey:@"firstLaunch"] isEqualToString:@"YES"];
    if (!userl || !defal) {
...
- (void)useNotificationWithString:(NSNotification *)notification //use notification method and logic
{
    ...
    {
        UINavigationController *nav = self.tabBarController.selectedViewController = [self.tabBarController.viewControllers objectAtIndex:4];
        
        [self.menuContainerViewController setMenuState:MFSideMenuStateClosed completion:^{}];
        [[nav.viewControllers objectAtIndex:0] editProfileBtnTapped];
    ...
    else if ([stringValueToUse isEqualToString:@"ProfileOpen_"])
    {
       // UINavigationController *nav = self.tabBarController.selectedViewController = [self.tabBarController.viewControllers objectAtIndex:4];
    ...
    else if ([stringValueToUse isEqualToString:@"ChatOpen"])
   {
//        UINavigationController *nav = self.tabBarController.selectedViewController = [self.tabBarController.viewControllers objectAtIndex:4];
//
...
//        [self.menuContainerViewController setMenuState:MFSideMenuStateClosed completion:^{}];
    ...
    else if ([stringValueToUse isEqualToString:@"ProfileEdit"]) {
       UINavigationController *nav = self.tabBarController.selectedViewController = [self.tabBarController.viewControllers objectAtIndex:4];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == 99) {
        if (buttonIndex == 0) {
            ...
            TOWebViewController *webViewController = [[TOWebViewController alloc] initWithURL:[NSURL URLWithString:@"http://www.stora.co/Sidetone/termsofuse.htm"]];
