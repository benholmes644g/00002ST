...
-(void)tryLoadingData{
    ...
    if(myInstance.fetching){
        [self performSelector:@selector(tryLoadingData) withObject:nil afterDelay:1];
    }else{
        [self loadTableView];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < self.dataset.count) {
        PFObject *activity = [self.dataset objectAtIndex:indexPath.row];
        if ([activity objectForKey:kESActivityPhotoKey]) {
            ...
            if ([photo objectForKeyedSubscript:@"videoThumbnail"]) {
                ...
                photoViewController.from=[activity objectForKey:kESActivityFromUserKey];
...
//    });
    ...
    if(myInstance.fetching){
        ...
        unreadCount = [myInstance.fromUser count] + [myInstance.toUser count];
