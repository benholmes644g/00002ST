...
@synthesize likeBarView,pinBarView;
@synthesize likeButton;
@synthesize delegate;
@synthesize currentLikeAvatars;
@synthesize clockView, locationView,photoImageViewButton,scroller;
...
- (void)createView {
    ...
    self.scroller =[[UIScrollView alloc] initWithFrame:CGRectMake(mainImageX, mainImageY, mainImageWidth, mainImageHeight)];
    [self.scroller setContentSize:self.photoImageView.frame.size];
    ...
    [self.photoImageViewButton setEnabled:NO];
    ...
    if ([[self.photo objectForKey:@"type"] isEqualToString:@"text"]) {
        ...
        [self.scroller addSubview:self.photoImageView];
        [self.scroller setContentSize:self.photoImageView.bounds.size];
    ...
    [self.photographer fetchIfNeededInBackgroundWithBlock:^(PFObject *object, NSError *error) {
        ...
        UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 120, 12.0f, 40, 18.0f)];
